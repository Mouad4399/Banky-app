...
class Transfer_Amount(QThread):
    ...
    def run(self):
        try:
            response=requests.post("http://127.0.0.1:8000/amount-transfer-process/",data=self.data,headers={'Authorization': f'Token {self.main_app.user_info.get("token")}'})
