...
class Get_Transactions_Summary(QThread):
    ...
    def run(self):
        try:
            response=requests.get("http://127.0.0.1:8000/transactions_summary/",data=self.data,headers={'Authorization': f'Token {self.main_app.user_info.get("token")}'})
